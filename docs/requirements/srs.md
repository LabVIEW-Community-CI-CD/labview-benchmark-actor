...
- Change Guidance: Sub-requirement of LBA-REQ-023 (ADR-0016). The verify-before-consume engine
  ...
  gated by `acg-provenance-verify-before-consume`). Flips to Proven when the sigstore-keyless path,
  rekor/mesh storage, and the reviewer-workstation verify-before-install wiring land. Authored under
  the `repo-standards-review` singular-requirement directive (one `shall`).
