...
- Change Guidance: Sub-requirement of LBA-REQ-023 (ADR-0016). The verify-before-consume engine
  ...
  `acg-keyless-attest-workflow-wired`). Flips to Proven on a live keyless-attest run demonstrating the
  Fulcio/rekor evidence -- the CI step, which needs OIDC + network and cannot be gated offline. Authored under
  the `repo-standards-review` singular-requirement directive (one `shall`).
