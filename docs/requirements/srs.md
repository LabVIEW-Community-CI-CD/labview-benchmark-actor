...
- Rationale: The golden VM exists to enable objective, reproducible cross-plane comparison
  ...
  arc (roadmap Phase 4), proven Linux-first.
- Acceptance Criteria:
  ...
  - Live evidence (Linux-first): VI Analyzer (host + scratch VM) and Mass Compile of the
    icon-editor `resource/` source (host + lba-golden) each agree on identity across two
    planes; the compile-time delta (39s host vs 24s VM) is the performance metric.
