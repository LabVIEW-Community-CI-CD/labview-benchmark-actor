...
- Change Guidance: Sub-requirement of LBA-REQ-023 (ADR-0016). The verify-before-consume engine
  ...
  logIndex 2311970781, recorded in `experiments/acg-transparency/keyless-attest-evidence.json`). Flips to
  Proven once the same workflow attaches the bundle on a real cut release (the release-publish path is wired;
  firing it is a maintainer step). Authored under
  the `repo-standards-review` singular-requirement directive (one `shall`).
