...
| T-029 | LBA-REQ-029 | Deterministic (offline) | Assert the ACG MCP surface (`grid-tools.mjs`) exposes the grid operations over the JSON-RPC 2.0 MCP contract: `initialize` returns the protocol + serverInfo, `tools/list` publishes the ADR-0020 tools (`spin_up_witness`/`run_quorum`/`get_confidence`/`verify_attestation`/`teardown`), and `tools/call` composes the engines (run_quorum corroborates; get_confidence returns the graded confidence; spin_up_witness/teardown return provisioning plans), failing closed on an unknown tool (-32602) with tool-execution failures riding `isError` in the result envelope. Also spawn `server.mjs` and assert a real stdin/stdout JSON-RPC round-trip. Self-test 10/10 gated by `acg-mcp-grid-surface` in `verify-local-gates`. |
