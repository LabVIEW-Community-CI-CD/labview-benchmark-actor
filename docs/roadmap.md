...
- **Phase 1 — Personal Golden VM (Linux)** *(the First Win, `LBA-REQ-033`, ADR-0023)*: one-command provision → human activation → `LabVIEWCLI` activation receipt → local golden VM minted + registered as an actor. Ubuntu 24.04 Noble; VirtualBox+Vagrant and Hyper-V/WSL2.
