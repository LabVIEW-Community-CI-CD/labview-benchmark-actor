...
| Flows through the shipped boundary | host materializes `fetched/<actorId>/<runId>/metrics.json` + `fetched/manifest.json` (`corpus-manifest@v1`, relative metricsRefs) and ingests it through `concentrateManifest()` — no hand editing |
...
| Out-of-band metric dereference | `dereferenceMetrics(corpus, fetched/)` reads each run's VM-local metrics file (a path in the manifest) into a real `cpuMean=…pct, ramMeanMiB=…, durationMs=…` summary |
...
| Drive-ready | `receipt.driveReady == true` + `receipt.driveCommand` points the maintainer at `drive-real-corpus.mjs --manifest fetched/manifest.json` |
...
the host only by this explicit out-of-band step). `fetched/` is git-ignored (regenerated per run).
...
- [export-corpus.ps1](export-corpus.ps1) — host driver: ensures the two VMs are up, runs the per-actor
  ...
  WinRM, materializes the `fetched/<actorId>/<runId>/metrics.json` layout + `fetched/manifest.json`
...
node experiments/ollama-comparison/drive-real-corpus.mjs \
  --manifest experiments/multi-vm-topology/corpus-export/fetched/manifest.json --model llama3.1:8b
```
