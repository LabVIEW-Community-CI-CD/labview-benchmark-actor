...
export const ACG_GRID_TOOLS = [
  ...
  { name: 'run_quorum', description: 'Run the corroboration quorum (ADR-0015) over a set of witness bundles and return the graded-majority verdict.', inputSchema: { type: 'object', properties: { bundles: { type: 'array', items: { type: 'object' } }, threshold: { type: 'number' } }, required: ['bundles'], additionalProperties: false } },
  { name: 'get_confidence', description: 'Return just the quorum verdict + graded confidence for a set of witness bundles.', inputSchema: { type: 'object', properties: { bundles: { type: 'array', items: { type: 'object' } }, threshold: { type: 'number' } }, required: ['bundles'], additionalProperties: false } },
  { name: 'verify_attestation', description: 'Verify-before-consume (ADR-0016): given attested witnesses + an enrolled allowlist, decide whether the release is consumable.', inputSchema: { type: 'object', properties: { witnesses: { type: 'array', items: { type: 'object' } }, allowlist: { type: 'object' } }, required: ['witnesses'], additionalProperties: false } },
  { name: 'check_independence', description: 'Assess witness independence (ADR-0017): whether the witnesses span distinct enrolled environments with recorded identities.', inputSchema: { type: 'object', properties: { witnesses: { type: 'array', items: { type: 'object' } }, enrollment: { type: 'object' } }, required: ['witnesses'], additionalProperties: false } },
