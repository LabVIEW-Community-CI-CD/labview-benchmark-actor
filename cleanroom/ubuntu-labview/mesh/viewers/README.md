...
Three host-side ways to observe the Ubuntu mesh's `lbabus net` coordination (TCP bus 7420 + UDP presence
7421 — the ports the lbabus binary actually uses; `net listen` binds 7420, `net beacon` sends 7421),
reading endpoints from the gitignored [`../../mesh-actors.csv`](../../mesh-actors.csv.example).
...
2. **Allow inbound UDP presence** (Windows Firewall blocks it by default):
   `New-NetFirewallRule -DisplayName 'lba-mesh-udp-7421' -Direction Inbound -Protocol UDP -LocalPort 7421 -Action Allow`
...
```bash
python3 /tmp/presence-beacon.py actor1 7421 60 192.168.56.1   # unicast presence to the host observer
```
